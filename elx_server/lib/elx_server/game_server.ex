defmodule ElxServer.GameServer do
  ...
  defmodule State do
    ...
    defstruct grid: %{},
              ...
              bombs: [],
              explosions: [],
              updated_players: %MapSet{},
              updated_cells: %MapSet{}
  ...
  defp diff_or_idle(%State{updated_players: up, updated_cells: uc} = state, started_at)
       when map_size(up) > 0 or map_size(uc) > 0 do
    updated_players =
      Enum.map(up, fn id -> state.players |> Map.fetch!(id) |> Player.snapshot() end)

    msg = %{
      "type" => @event_diff,
      "updatedPlayers" => updated_players,
      "updatedCells" => Enum.map(uc, &%{&1 | value: Cell.to_int(&1.value)}),
    ...
    {:noreply, %State{state | updated_players: %MapSet{}, updated_cells: %MapSet{}}}
  end
